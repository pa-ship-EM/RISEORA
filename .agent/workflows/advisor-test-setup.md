...
npx tsx -e "import { storage } from './server/storage'; async function run() { const u = await storage.createUser({ email: 'test_advisor@example.com', password: 'password123', firstName: 'Test', lastName: 'Advisor', role: 'AFFILIATE', betaAccessCode: 'RO-PRODUCTIONBETA0' }); console.log('Advisor created:', u.id); } run();"
```
