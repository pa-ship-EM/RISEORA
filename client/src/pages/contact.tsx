...
export default function Contact() {
    ...
    const content = (
        <main className={`${user ? "py-4" : "py-20 flex-grow"} bg-slate-50`}>
            <div className={`${user ? "" : "container mx-auto px-4"}`}>
                <div className={`text-center max-w-2xl mx-auto ${user ? "mb-8 text-left max-w-none" : "mb-16"}`}>
                    <h1 className={`${user ? "text-2xl" : "text-4xl md:text-5xl"} font-serif font-bold text-primary mb-6`}>
                        {user ? "Contact Support" : "Get in Touch"}
                    </h1>
                    <p className={`${user ? "text-base" : "text-xl"} text-muted-foreground`}>
                        {user
                            ? "Have questions about your disputes? Our compliance team is here to help."
                            : "Have questions? We're here to help you on your journey to financial freedom."
                        }
                    </p>
                </div>

                <div className={`grid ${user ? "lg:grid-cols-3" : "lg:grid-cols-2"} gap-12 max-w-6xl mx-auto`}>
                    {/* Contact Form */}
                    <motion.div
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        className={`bg-white rounded-2xl p-8 shadow-sm border border-slate-200 ${user ? "lg:col-span-2" : ""}`}
                    >
                    ...
                    >
                        ...
                        {user && (
                            <div className="bg-primary/5 rounded-2xl p-6 border border-primary/10 flex items-start gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-primary mb-1">Secure Support</h3>
                                    <p className="text-sm text-muted-foreground leading-relaxed">
                                        Support requests are handled through our encrypted server to ensure your dispute privacy.
                                    </p>
                                </div>
                            </div>
                        )}
